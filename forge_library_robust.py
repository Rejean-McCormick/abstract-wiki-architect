...
def forge_symbolic(lang_code):
    ...
    # FIX: Open Paradigms to access mkPN.
    # FIX: Use \s -> mkNP (mkPN s) pattern which is universally supported.
    content = f"""
...
def forge_dictionary(lang_code):
    ...
    print(f"   🔨 Forging Dict{lang_code}.gf (Robust Mode)...")

    # Header opens Prelude to access 'ss'
    header = f"""
resource Dict{lang_code} = open Cat{lang_code}, Paradigms{lang_code}, Syntax{lang_code}, Prelude in {{
  oper
    -- Robust constructor for Adverbs to bypass strict RGL rules
    mkAdvS : Str -> Adv = \\s -> lin Adv (ss s) ;
    mkAS   : Str -> A   = \\s -> lin A (ss s) ;
    mkNS   : Str -> N   = \\s -> lin N (ss s) ;
    mkVS   : Str -> V   = \\s -> lin V (ss s) ;
"""
    body = ""
    for key in REQUIRED_LEXICON:
        ...
        # Use our Safe Constructors for everything to be absolutely sure
        if cat == "N":
            body += f"    {func_name} = mkNS \"{word}\" ;\n"
        elif cat == "A":
            body += f"    {func_name} = mkAS \"{word}\" ;\n"
        elif cat == "V":
            body += f"    {func_name} = mkVS \"{word}\" ;\n"
        elif cat == "Adv":
...
def run():
    print("🚀 Starting Robust Library Forge (v2)...")
